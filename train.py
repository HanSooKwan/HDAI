...
from Dataloader import HeartDataset
from shape_attentive_unet.data.augmentations import Compose, RandomSizedCrop, AdjustContrast, AdjustBrightness, RandomVerticallyFlip, \
    RandomHorizontallyFlip, RandomRotate, PaddingCenterCrop
# Our libs
from shape_attentive_unet.data.ac17_dataloader import AC17Data as AC17, AC17_2DLoad as load2D
from shape_attentive_unet.models import ModelBuilder, SegmentationModule
from shape_attentive_unet.utils import AverageMeter, parse_devices, accuracy, intersectionAndUnion
from shape_attentive_unet.lib.nn import UserScatteredDataParallel, async_copy_to, user_scattered_collate, patch_replication_callback
import shape_attentive_unet.lib.utils.data as torchdata
from shape_attentive_unet.lib.utils import as_numpy
import numpy as np
from shape_attentive_unet.loss import DualLoss
from shape_attentive_unet.radam import RAdam


def eval(loaders_val, segmentation_module, args, crit):
    intersection_meter = AverageMeter()
    union_meter = AverageMeter()
    loss_meter = AverageMeter()

    segmentation_module.eval()
    for loader_val in loaders_val:
        for batch_data in loader_val:
            # Add this line
            batch_data = np.stack([batch_data]*3, axis=1)
...
# train one epoch
def train(segmentation_module, loaders_train, optimizers, history, epoch, args):
    ...
    for loader_train in loaders_train:
        for batch_data in loader_train:
            data_time.update(time.time() - tic)
            batch_data["image"] = batch_data["image"].cuda() # Cropped image. uncropped image is "image_before_crop"
            batch_data["image"] = torch.stack([batch_data["image"]]*3, dim=1)
            # it is already cropped
...
def main(args):
    ...
    train_augs = Compose([RandomHorizontallyFlip(), RandomVerticallyFlip(), RandomRotate(180)])
    test_augs = Compose([])

    # # Dataset and Loader for AC17 Dataset
    ...
    #
    # AC17_loader_train = data.DataLoader(
    #     ac17_train,
    ...
    #
    # AC17_ac17_val = load2D(dataset_val, split='val', deform=False)
    #
    ...
    cropper = { # Row min, Row max, Col min, Col max
        "434_636": (41,430,50,610),
        "708_1016": (80,660,96,920),
        "600_800": (70,570,106,765),
        "720_960": (90,590,125,830),
        "422_636": (41,420,10,621),
        "768_1024": (81,700,40,1000),
        "480_640": (74,444,50,580)
    }
    # Load train dataset to dataloader
    # train_datasets = []
    train_dataloaders = []
    for shape_string in cropper.keys():
        heart_dataset = HeartDataset(
            shape_string=shape_string,
            # transform=train_augs,
            train=True
        )
        # train_datasets.append(heart_dataset)
    # heart_dataset_train = torch.utils.data.ConcatDataset(train_datasets)
        train_dataloaders.append(data.DataLoader(
            heart_dataset,
            batch_size=4,
            shuffle=True,
            num_workers=int(args.workers),
    ...
    # Load validation dataset to dataloader
    # val_datasets = []
    val_dataloaders = []
    for shape_string in cropper.keys():
        heart_dataset = HeartDataset(
        ...
        # val_datasets.append(heart_dataset)
    # heart_dataset_val = torch.utils.data.ConcatDataset(val_datasets)
        val_dataloaders.append(data.DataLoader(
            heart_dataset,
            batch_size=1,
            shuffle=False,
            collate_fn=user_scattered_collate,
    ...
    for epoch in range(args.start_epoch, args.num_epoch + 1):
        train(segmentation_module, train_dataloaders, optimizers, history, epoch, args)
        iou, loss = eval(val_dataloaders, segmentation_module, args, crit)
        # checkpointing
        ckpted = False
...
if __name__ == '__main__':
    ...
    DATA_ROOT = os.getenv('data', '/PATH/TO/AC17/DATA')
    DATASET_NAME = "AC17"
    ...
    parser.add_argument('--dataset-name', type=str, default="AC17+HDAI")
